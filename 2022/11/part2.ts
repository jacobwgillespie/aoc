...
async function main() {
  ...
  const common = monkeys.map((m) => m.divisibleBy).reduce((a, b) => a * b)

  for (let i = 0; i < 10000; i++) {
    for (const monkey of monkeys) {
      let item = monkey.startingItems.shift()
      while (item !== undefined) {
        const newItem = monkey.operation(item) % common
...
function parseMonkey(input: string): Monkey {
  ...
  const divisibleBy = Number(lines[3].split(': ')[1].slice(13))
  const test = parseTest(lines[3].split(': ')[1])
  const ifTrue = Number(lines[4].split(' ')[5])
  const ifFalse = Number(lines[5].split(' ')[5])
  return {number, startingItems, operation, divisibleBy, test, ifTrue, ifFalse, inspectedItemCount: 0}
}
